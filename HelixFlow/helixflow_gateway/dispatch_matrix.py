...
# Initialize OpenTelemetry Tracer for Application Insights
...
def calculate_routing_score(messages: List[MessageContract], available_providers: Optional[dict] = None) -> str:
    ...
    """
    with tracer.start_as_current_span("calculate_routing_matrix") as span:
        ...
        selected_fabric = _select_from_tier(tier, available_providers)
        
        # Telemetry: Emit span attributes to Azure App Insights
        span.set_attribute("gen_ai.system", "HelixFlowGateway")
        span.set_attribute("routing.tier", tier)
        span.set_attribute("routing.fabric_key", selected_fabric)
        span.set_attribute("routing.estimated_tokens", total_tokens)
