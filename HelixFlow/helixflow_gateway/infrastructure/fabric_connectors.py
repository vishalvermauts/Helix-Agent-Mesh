...
async def resolve_upstream(target_model: str, client_auth_header: Optional[str] = None, cache_client=None) -> tuple[str, dict, str]:
    ...
    # Azure AI Foundry Model Catalog Settings
    azure_foundry_base = os.environ.get("AZURE_FOUNDRY_BASE", "https://mock-azure-foundry.openai.azure.com/openai/deployments")
    azure_foundry_key = os.environ.get("AZURE_FOUNDRY_KEY", "mock-azure-foundry-api-key")
    azure_api_version = "2024-02-15-preview"

    # Edge Compute Node (Droplet 3)
    local_ollama_url = os.environ.get("LOCAL_OLLAMA_URL", "")
    ...
    if cache_client:
        try:
            ...
            if config_json:
                ...
                # Load Azure Foundry keys if provided
                azure_foundry_key = credentials.get("azure_foundry_key", azure_foundry_key)
                azure_foundry_base = credentials.get("azure_foundry_base", azure_foundry_base)
    ...
    if target_model == "fabric-speed-edge":
        ...
        # Provisioned from Azure AI Foundry Model Catalog
        deployment_id = "gpt-4o"
        if target_model == "fabric-openai-gpt4o-mini":
            deployment_id = "gpt-4o-mini"
            
        if azure_foundry_key == "mock-azure-foundry-api-key":
            # FALLBACK FOR DEMO VIDEO: Route to standard OpenAI to prevent crash
            url = f"{runtime_settings.DEFAULT_FABRIC_URL.rstrip('/')}/chat/completions"
            headers["Authorization"] = f"Bearer {client_auth_header.replace('Bearer ', '') if client_auth_header else ''}"
            model_name = deployment_id
        else:
            url = f"{azure_foundry_base.rstrip('/')}/{deployment_id}/chat/completions?api-version={azure_api_version}"
            model_name = deployment_id
            
            # Adaptive Circuit Breaker Fallback
            headers["api-key"] = azure_foundry_key
            if client_auth_header and not azure_foundry_key:
                headers["api-key"] = client_auth_header.replace("Bearer ", "")

    elif target_model.startswith("claude-") or "anthropic" in target_model or target_model == "fabric-anthropic-claude":
        # Provisioned from Azure AI Foundry Model Catalog
        deployment_id = "claude-3-5-sonnet"
        if azure_foundry_key == "mock-azure-foundry-api-key":
             # FALLBACK FOR DEMO VIDEO: Route to Anthropic standard
             url = "https://api.anthropic.com/v1/messages"
             headers["x-api-key"] = client_auth_header.replace("Bearer ", "") if client_auth_header else ""
